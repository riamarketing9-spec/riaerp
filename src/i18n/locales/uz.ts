export const uz = {
  common: {
    all: 'Barchasi',
    loading: 'Yuklanmoqda',
    logout: 'Chiqish',
    save: 'Saqlash',
    cancel: 'Bekor qilish',
    create: 'Yaratish',
    edit: "O'zgartirish",
    delete: "O'chirish",
    search: 'Qidiruv',
    toggleTheme: "Yorug'/qorong'i mavzu",
    saving: 'Saqlanmoqda…',
    saved: 'Saqlandi',
    saveError: 'Saqlanmadi — xatolik',
    done: 'Tayyor',
    yes: 'Ha',
    no: "Yo'q",
  },
  auth: {
    signInTitle: 'Tizimga kirish',
    email: 'Email',
    password: 'Parol',
    signIn: 'Kirish',
    invalidCredentials: "Email yoki parol noto'g'ri",
  },
  nav: {
    dashboard: 'Boshqaruv paneli',
    projects: 'Loyihalar',
    tasks: 'Vazifalar',
    cabinet: 'Ish stolim',
    workload: 'Jamoa bandligi',
    contentPlan: 'Kontent reja',
    clients: 'Mijozlar',
    leads: 'Sotuv jarayoni',
    org: 'Tashkiliy tuzilma',
    docs: 'Hujjatlar',
    audit: 'Amallar jurnali',
    attendance: 'Xodimlar ish vaqti',
    lookupAdmin: "Ma'lumotnomalar",
  },
  attendance: {
    title: 'Xodimlar ish vaqti',
    widgetTitle: 'Ish vaqti',
    start: 'Ishni boshlash',
    stop: "Tugatish",
    working: 'Ishlamoqda',
    notWorking: 'Hozir ishlamayapsiz',
    currentlyWorking: 'Hozir ishlayotganlar',
    todayTitle: 'Bugungi davomat',
    notArrived: 'Kelmadi',
    noneWorking: "Hozircha hech kim ishlamoqda deb belgilanmagan",
    since: '-dan beri',
    forceStop: "Sessiyani yopish",
    alreadyStarted: "Siz allaqachon belgilangansiz — taymer allaqachon ishlayapti",
    period: 'Davr',
    today: 'Bugun',
    yesterday: 'Kecha',
    week: 'Hafta',
    month: 'Oy',
    range: 'Davr',
    from: 'Dan',
    to: 'Gacha',
    employee: 'Xodim',
    allEmployees: 'Barcha xodimlar',
    history: 'Tarix',
    leaderboard: "Kim ko'proq ishladi",
    empty: "Bu davr uchun yozuvlar yo'q",
  },
  workloadPage: {
    startsAfter: 'Vazifalar boshlangan sana',
  },
  dashboard: {
    deadlines: 'Muddatlar',
    overdue: "Muddati o'tgan",
    dueSoon: 'Muddati yaqinlashgan',
    idleTeam: 'Bo\'sh xodimlar',
    idleTeamEmpty: 'Hammasi band',
    idleTeamFree: "Bo'sh",
    idleTeamTotal: 'Jami',
    finance: 'Moliya',
    expectedRevenue: 'Kutilayotgan daromad (oy)',
    netProfit: 'Sof foyda',
    todayContent: 'Bugungi nashrlar',
    todayContentEmpty: "Bugun nashr yo'q",
    teamTasks: 'Jamoa vazifalari',
    teamTasksEmpty: "Jamoada hozircha vazifalar yo'q",
    myOpenTasks: 'Mening ochiq vazifalarim',
    myOverdue: "Mening muddati o'tganlarim",
    completedThisWeek: '7 kunda bajarilgan',
    inProgress: 'Jarayonda',
    taskStatusChart: 'Vazifalar',
    taskStatusChartTeam: 'Jamoa vazifalari',
    byProjectChart: "Loyihalar bo'yicha vazifalar",
    showTable: 'Jadval',
    contentPlanChart: 'Mening kontent-rejam',
    contentPlanChartTeam: 'Jamoa kontent-rejasi',
    total: 'Jami',
  },
  audit: {
    title: 'Amallar jurnali',
    table: "Bo'lim",
    actor: 'Kim',
    action: 'Amal',
    date: 'Qachon',
    actionInsert: 'Yaratdi',
    actionUpdate: "O'zgartirdi",
    actionDelete: "O'chirdi",
    allTables: "Barcha bo'limlar",
    allEmployees: 'Barcha xodimlar',
    empty: "Hozircha yozuvlar yo'q",
    system: 'Tizim',
    details: "Nima o'zgardi",
  },
  projects: {
    title: 'Loyihalar',
    newProject: 'Yangi loyiha',
    name: 'Nomi',
    logo: 'Logotip',
    status: 'Holati',
    type: 'Turi',
    newType: 'Yangi tur',
    result: 'Natija',
    resultStories: 'Stories',
    resultPosts: 'Postlar (post + reels)',
    resultVideo: 'YouTube video',
    resultEdits: 'Qilingan montajlar',
    resultShoots: 'Qilingan syomkalar',
    progressDetail: 'Bu oyda nima qilindi',
    progressDetailEmpty: "Hozircha hech narsa",
    pm: 'PM',
    goal: 'Maqsad',
    audience: 'Maqsadli auditoriya (matn)',
    audienceVoice: 'Maqsadli auditoriya (ovozli)',
    audienceFile: 'Maqsadli auditoriya (PDF)',
    billingDay: "Hisob kuni",
    startDate: 'Boshlanish sanasi',
    endDate: 'Tugash sanasi',
    monthlyGoal: 'Oylik maqsad',
    goalPosts: 'Post (reels+post+karusel)',
    goalStories: 'Stories',
    goalAds: 'Target',
    goalNote: 'Izoh (ixtiyoriy)',
    addGoal: 'Oylik maqsad',
    client: 'Mijoz / voronka',
    deliverables: "Bizning qilib berish kerak bo'lgan ish",
    assistantPm: 'PM yordamchisi',
    contract: "Shartnoma (faqat CEO ko'radi)",
    contractCurrent: "Joriy shartnomani ochish",
    contractRemove: "Shartnomani o'chirish",
  },
  tasks: {
    title: 'Vazifalar',
    newTask: 'Yangi vazifa',
    assignee: 'Bajaruvchi',
    deadline: 'Muddat',
    priority: 'Prioritet',
    status: 'Holati',
    percentComplete: 'Bajarilgan %',
    deliverable: 'Natija',
    urgent: 'Shoshilinch',
    important: 'Muhim',
    empty: "Vazifalar hozircha yo'q",
    termType: 'Bajarish muddati',
    termAutoHint: "Muddatga qarab avtomatik tanlanadi, qo'lda o'zgartirish mumkin",
    assigneeReadOnlyHint: "Siz ijrochisiz — faqat holatni o'zgartira olasiz. Qolgan maydonlarni PM yoki CEO o'zgartiradi.",
    filterMine: 'Mening vazifalarim',
    filterTeam: 'Jamoa',
    filterByEmployee: 'Xodimni tanlash',
    dateFrom: 'Sanadan',
    dateTo: 'Sanagacha',
    resetDateFilter: 'Sanani tozalash',
    eisenhower: 'Prioritet (Eyzenxauer matritsasi)',
    recurrence: 'Takrorlanishi',
    checklist: 'Chek-list',
    newChecklistItem: 'Yangi chek-list vazifasi',
    checklistItemAdded: "Chek-listga qo'shildi",
    statusDurations: "Bosqichlar bo'yicha vaqt (PM/CEO ko'radi)",
    quadrantDoNow: 'Shoshilinch va muhim',
    quadrantSchedule: 'Shoshilinch emas, lekin muhim',
    quadrantDelegate: 'Shoshilinch, lekin muhim emas',
    quadrantEliminate: 'Shoshilinch va muhim emas',
    subtasks: 'Cheklist',
    addSubtask: "Band qo'shish",
    subtaskPlaceholder: 'Band nomi',
    comments: 'Izohlar',
    addComment: "Izoh qo'shish",
    commentPlaceholder: 'Izoh yozing...',
    details: 'Vazifa',
    save: 'Saqlash',
    startsAt: 'Boshlanishi',
  },
  cabinet: {
    title: 'Ish stolim',
    dailyChecklist: 'Bugungi checklist',
    myTasks: 'Mening vazifalarim',
    empty: "Hozircha vazifalar yo'q",
    telegramTitle: 'Telegram-bot',
    telegramInstructions: "Telefon almashtirdingizmi yoki ikkinchi akkaunt bormi? Tugmani yana bosing va yangi chatda /start yuboring — eski ulanishlar saqlanib qoladi.",
    telegramConnectButton: "Yana bitta Telegram ulash",
    telegramUnknownAccount: 'Nomsiz akkaunt',
  },
  workload: {
    title: 'Jamoa bandligi',
    openTasks: 'Ochiq vazifalar',
    limit: 'Limit',
    overloaded: 'Band',
    active: 'Faol',
    archive: 'Arxiv',
    noActive: 'Faol vazifalar yo\'q',
    noArchive: 'Tugallangan vazifalar yo\'q',
  },
  clients: {
    title: 'Mijozlar',
    newClient: 'Yangi mijoz',
    name: 'Nomi',
    contactName: "Bog'lanish shaxsi",
    contactPhone: 'Telefon',
    contactTelegram: 'Telegram',
    industry: 'Soha',
    status: 'Holati',
    empty: "Mijozlar hozircha yo'q",
    history: 'Ish tarixi',
    addNote: "Yozuv qo'shish",
    noteText: 'Nima bo\'ldi',
    emptyHistory: "Yozuvlar hozircha yo'q",
  },
  leads: {
    title: 'Sotuv jarayoni',
    newLead: 'Yangi lid',
    client: 'Mijoz',
    stage: 'Bosqich',
    owner: "Mas'ul",
    expectedValue: 'Kutilayotgan summa',
    nextActionDate: 'Keyingi qadam (sana)',
    notes: 'Izohlar',
    empty: "Lidlar hozircha yo'q",
  },
  org: {
    title: 'Tashkiliy tuzilma',
    newPosition: 'Yangi lavozim',
    positionTitle: 'Lavozim nomi',
    parent: "Bo'ysunadi",
    person: 'Xodim',
    none: '—',
    empty: "Tashkiliy tuzilma hali to'ldirilmagan",
    vacant: "Bo'sh o'rin",
    addSubordinate: "Bo'ysunuvchi qo'shish",
    addSibling: "Shu darajaga qo'shish",
    dragHandle: "Ko'chirish",
    dropToRoot: "Rahbarni olib tashlash uchun shu yerga tashlang",
    moved: "Lavozim ko'chirildi",
    cycleError: "Lavozimni o'z bo'ysunuvchisi ostiga ko'chirib bo'lmaydi",
  },
  docs: {
    title: 'Hujjatlar',
    newDocument: 'Yangi hujjat',
    newContract: 'Yangi shartnoma',
    docTitle: 'Nomi',
    category: 'Kategoriya',
    storagePath: 'Fayl / havola',
    note: 'Izoh',
    employee: 'Xodim',
    team: 'Jamoa',
    assignTo: 'Kimga',
    allEmployees: 'Barcha xodimlar',
    tabDocuments: 'Hujjatlar',
    tabContracts: 'Shartnomalar',
    orgWide: 'Barcha xodimlarga ko\'rinadi',
    grantAccess: 'Ruxsat berish',
    revokeAccess: 'Ruxsatni bekor qilish',
    accessRevoked: 'Ruxsat bekor qilindi',
    empty: "Hujjatlar hozircha yo'q",
    emptyContracts: "Shartnomalar hozircha yo'q",
  },
  finance: {
    title: 'Moliya',
    revenue: 'Daromad',
    expenses: 'Xarajatlar',
    payroll: 'Oyliklar',
    newRevenue: "Daromad qo'shish",
    newExpense: "Xarajat qo'shish",
    project: 'Loyiha',
    month: 'Oy',
    amount: 'Summa',
    date: 'Sana',
    category: 'Kategoriya',
    scope: 'Turi',
    scopeBusiness: 'Biznes',
    scopePersonal: 'Shaxsiy',
    note: 'Izoh',
    emptyRevenue: "Daromad hozircha yo'q",
    emptyExpenses: "Xarajat hozircha yo'q",
    profit: 'Foyda',
    totalRevenue: 'Jami daromad',
    totalExpenses: 'Jami xarajat',
  },
  payroll: {
    title: 'Oyliklar',
    fixedSalary: 'Belgilangan oyliklar',
    rates: 'Sdelka stavkalari',
    newFixedSalary: "Oylik qo'shish",
    newRate: "Stavka qo'shish",
    employee: 'Xodim',
    monthlyAmount: 'Oylik summa',
    deliverableType: 'Ish turi',
    rate: 'Stavka',
    effectiveFrom: 'Amal qilish sanasi',
    emptyFixed: "Oyliklar hozircha yo'q",
    emptyRates: "Stavkalar hozircha yo'q",
    runs: 'Oylik hisob-kitoblar',
    generateRun: 'Hisob-kitob yaratish',
    month: 'Oy',
    finalize: 'Tasdiqlash',
    draft: 'Qoralama',
    finalized: 'Tasdiqlangan',
    total: 'Jami',
    emptyRuns: "Hisob-kitoblar hozircha yo'q",
  },
  kpi: {
    title: 'CEO paneli',
    mrr: 'MRR (bu oydagi daromad)',
    activeProjects: 'Faol loyihalar',
    overdueTasks: 'Muddati o\'tgan vazifalar',
    projectProfit: 'Loyihalar bo\'yicha foyda',
    employeeKpi: 'Xodimlar KPI',
    tasksCompleted: 'Bajarilgan vazifalar',
    onTime: "O'z vaqtida",
    avgPercent: "O'rtacha bajarilish",
    overloadedEmployees: 'Ortiqcha yuklangan xodimlar',
    revenueProfitTrend: "6 oylik daromad va foyda",
    expenseBreakdown: "Xarajatlar tarkibi (shu oy)",
    expenseBreakdownEmpty: "Bu oyda hozircha xarajat yo'q",
    totalExpenses: 'Jami xarajatlar',
    otherCategory: 'Boshqa',
    reportsTitle: 'KPI va Hisobotlar',
    byEmployee: "Xodim bo'yicha",
    byProject: "Loyiha bo'yicha",
    pickEmployee: 'Xodimni tanlang',
    pickProject: 'Loyihani tanlang',
    onTimePct: "O'z vaqtida bajarilgan, %",
    avgPercentComplete: "Vazifalarning o'rtacha bajarilishi",
    onTimeCount: 'Muddatida ulgurgan',
    fullyDoneCount: "To'liq bajarilgan (100%)",
    lateList: 'Muddati bo\'lgan vazifalar bo\'yicha kechikishlar',
    onTimeBadge: 'Vaqtida',
    projectPercentDone: 'Loyiha % bajarilgani',
    monthPosts: 'Bu oy postlar (reels+post+karusel)',
    monthStories: 'Bu oy stories',
    monthTarget: 'Bu oy target',
    planned: 'reja',
    clientReport: 'Mijoz hisoboti',
    noMonthlyGoal: "Bu oy uchun maqsad hali belgilanmagan",
  },
  contentPlan: {
    title: 'Kontent reja',
    newItem: 'Yangi kontent',
    topic: 'Mavzu',
    project: 'Loyiha',
    format: 'Format',
    deliverableTypeHint: "Bir nechtasini tanlash mumkin — har biri alohida hisoblanadi",
    platforms: 'Platformalar',
    status: 'Holati',
    responsible: "Mas'ul",
    shooter: 'Syomkachi',
    editor: 'Montajchi',
    smm: 'SMMchi',
    shootDate: 'Syomka sanasi',
    editDoneDate: 'Montaj sanasi',
    coverDoneDate: 'Cover (muqova) sanasi',
    publishDate: 'Chiqish sanasi',
    empty: "Kontent reja hozircha yo'q",
    script: 'Ssenariy',
    tor: 'Dizayner/montajchi uchun TZ',
    rubric: 'Rubrika',
    videoGoal: 'Video maqsadi',
    referenceUrl: 'Havola/referens',
    calendarView: 'Kalendar',
    foldersView: 'Papkalar',
    tableView: 'Jadval',
    today: 'Bugun',
    deliverableType: 'Ish turi (oylik hisobi uchun)',
    filters: 'Filtrlar',
    allProjects: 'Barcha loyihalar',
    searchProjects: 'Loyiha nomi bo\'yicha qidiruv',
    allPlatforms: 'Barcha platformalar',
    allFormats: 'Barcha formatlar',
    allStatuses: 'Barcha holatlar',
    dateFrom: 'Sanadan',
    dateTo: 'Sanagacha',
    resetFilters: 'Tozalash',
    save: 'Saqlash',
    details: 'Kontent kartasi',
  },
  kanban: {
    title: 'Vazifalar kanban',
  },
  kb: {
    title: 'Bilimlar bazasi',
    newArticle: 'Yangi maqola',
    articleTitle: 'Nomi',
    body: 'Matn',
    videoUrl: 'Video havolasi',
    empty: "Maqolalar hozircha yo'q",
    markRead: 'Tanishib chiqdim',
    readAt: "O'qildi",
    readCount: "O'qiganlar",
  },
  team: {
    title: 'Jamoa',
    newEmployee: "Xodim qo'shish",
    fullName: 'F.I.Sh.',
    email: 'Email',
    password: 'Parol',
    role: 'Roli',
    telegramLink: 'Telegram ulash havolasi',
    telegramConnected: 'Telegram ulangan',
    telegramConnectedCount: 'Telegram: {{count}}',
    telegramNotConnected: 'Telegram ulanmagan',
    copyLink: 'Havolani nusxalash',
    linkCopied: 'Havola nusxalandi',
    department: "Bo'lim",
    status: 'Holati',
    showInactive: "Faol bo'lmaganlarni ko'rsatish",
    editEmployee: 'Xodim',
    confirmDelete: "{{name}} xodimini jamoadan o'chirmoqchimisiz? ERP ga kirish darhol yopiladi, yozuv o'chirilganlar tarixida qoladi.",
    saved: 'Saqlandi',
    apprentice: 'Shogird (o\'quvchi)',
    apprenticeBadge: 'Shogird',
    history: "O'chirilganlar tarixi",
    showHistory: "O'chirilganlar tarixini ko'rsatish",
    hideHistory: "O'chirilganlar tarixini yashirish",
    historyEmpty: "O'chirilgan xodimlar yo'q",
    deletedOn: "O'chirilgan sana: {{date}}",
    deletedBy: "Kim o'chirdi: {{name}}",
    deletedByUnknown: "noma'lum",
    departments: "Bo'limlar",
    newDepartment: "Bo'lim qo'shish",
    departmentNameRu: 'Nomi (rus.)',
    departmentNameUz: 'Nomi (o\'zb.)',
    departmentAdded: "Bo'lim qo'shildi",
    departmentDeactivated: "Bo'lim faolsizlantirildi",
    secondaryRoles: "Qo'shimcha lavozimlar",
    secondaryRolesHint: "Agar xodim bir nechta rolni bajarsa — shu yerda belgilang. Har bir belgilangan lavozimning ruxsatlari asosiy rol ruxsatlariga qo'shiladi",
    permissions: 'Ruxsatlar',
    permissionsHint: "Standart holatda — roli bo'yicha ruxsatlar; ushbu xodim uchun aniq bir ruxsatni qo'shish yoki olib tashlash uchun belgilang",
    permissionFromRole: "roli bo'yicha",
    permissionCustom: "qo'lda",
    capOrgFullAccess: 'Butun tizimga to\'liq kirish',
    capFinanceRead: "Moliyani ko'rish",
    capFinanceWrite: 'Moliyani tahrirlash',
    capCabinetsReadAll: "Barcha xodimlar/kabinetlarni ko'rish",
    capProjectsManage: 'Loyihalarni boshqarish',
    capProjectsReadScoped: "O'z loyihalarini ko'rish",
    capContentPlanManage: "Kontent-rejaga to'liq kirish (barcha loyihalar)",
    capSalesRead: "Sotuvlarni ko'rish",
    capSalesManage: 'Sotuvlarni boshqarish',
    capOrgStructureManage: 'Tashkiliy tuzilmani tahrirlash',
    capDocsAdmin: 'Hujjatlar va bilimlar bazasi',
    capTeamManage: "Jamoani boshqarish (taklif qilish/ruxsatlar)",
    capProjectsManageHint: "Barcha loyihalarga birdaniga qo'llaniladi, bittasiga emas",
  },
  backup: {
    export: 'Backup yuklab olish',
    exporting: 'Backup tayyorlanmoqda...',
  },
  aiReport: {
    title: 'Mijozga hisobot (AI)',
    description: "Davrni tanlang — AI shu davrdagi chop etilgan kontent va bajarilgan vazifalarni yig'ib, mijozga yuborsa bo'ladigan tayyor hisobot matnini tuzadi (yuklab olish mumkin).",
    generate: 'Hisobot yaratish',
    periodStart: 'Davr boshi',
    periodEnd: 'Davr oxiri',
    generating: 'Yaratilmoqda...',
  },
  lookupManager: {
    title: "Ma'lumotnomalar",
    selectTable: "Ma'lumotnoma",
    slug: 'Identifikator (slug)',
    slugAutoHint: "Avtomatik to'ldiriladi",
    labelRu: 'Nomi (rus)',
    labelUz: 'Nomi (uzb)',
    dayMin: 'Min. kun',
    dayMax: 'Maks. kun',
    weight: "Og'irlik",
    add: "Qo'shish",
    added: "Yozuv qo'shildi",
    empty: 'Hozircha yozuvlar yo\'q',
    tables: {
      contentRubrics: 'Kontent rubrikalari',
      contentFormats: 'Kontent formatlari',
      platforms: 'Platformalar',
      deliverableTypes: 'Ish natijalari turlari',
      clientStatuses: 'Mijoz statuslari',
      leadStages: 'Sotuv jarayoni bosqichlari',
      expenseCategories: 'Xarajat toifalari',
      documentCategories: 'Hujjat toifalari',
      staffStatuses: 'Xodim statuslari',
      contractTypes: 'Shartnoma turlari',
      projectTypes: 'Loyiha turlari',
      projectStatuses: 'Loyiha statuslari',
      taskStatuses: 'Vazifa statuslari',
      contentStatuses: 'Kontent statuslari',
      taskTermTypes: 'Vazifa muddat toifalari',
      taskPriorityQuadrants: 'Ustuvorlik kvadrantlari (Eyzenxauer)',
    },
  },
}
